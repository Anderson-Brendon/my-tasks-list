...
<section class="taskContainerColor col-9 col-sm-6 col-md-4 col-lg-3 col-xl-3 m-3 p-1 border rounded border-3">
        <div class="d-flex flex-column align-items-center">
            ...
            @if ($isDoneToday)
            <div class="spinner-grow text-success float-start" role="status">
                <span class="visually-hidden">Loading...</span>
            </div>
            <p>Status: Completada</p>
            @else
            <div class="spinner-grow text-warning float-start" role="status">
                <span class="visually-hidden">Loading...</span>
            </div>
            <p>Status: Pendente</p>
            @endif
            ...
            <span>@if($task->last_completed != null){{-- pq aqui nao da pra usar o metodo--}}
            ...
            <p class="greenyellow">Número de vezes completada: {{ $task->completed_number }}</p>
        </div>
    <div class="d-flex flex-column justify-content-around">
        <div class="d-flex justify-content-around">
            ...
            <form onsubmit="return confirm('Tem certeza?')"
                action="{{ route('daily.task.destroy', $task->id_daily_task) }}" method="POST">
                @csrf
                @method('DELETE')
